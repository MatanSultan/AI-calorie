        ...
        <div className="grid gap-5 px-5 py-6 sm:px-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge className="bg-white/15 text-white hover:bg-white/15" variant="default">
                {new Date(detail.meal.occurred_at).toLocaleString("he-IL")}
              </Badge>
            </div>
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-cyan-50">
                הערכים נשמרו לאחר אישור הארוחה. אפשר לראות כאן את פירוט הפריטים, התמונה, וסיכום החידוד עם ה-AI.
              </p>
            </div>
          </div>

          <div className="rounded-[28px] border border-white/15 bg-slate-950/15 p-4 backdrop-blur">
            <p className="text-sm font-semibold text-white/90">סה"כ קלוריות לארוחה</p>
            <p className="mt-2 text-4xl font-bold">{Math.round(totalCalories)} kcal</p>
            <p className="mt-2 text-sm text-cyan-50">הסכום נשמר תחת הארוחות המאושרות בלבד.</p>
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-4">
          <Card className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle>פירוט פריטים</CardTitle>
                <CardDescription>רשימת הפריטים שנשמרו יחד עם הכמות והקלוריות.</CardDescription>
              </div>
            </div>
          ...
          <Card className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle>סיכום חידוד</CardTitle>
                <CardDescription>אם דייקת את הארוחה דרך הצ'אט, הסיכום נשמר כאן.</CardDescription>
              </div>
            </div>

            <div className="rounded-[24px] bg-slate-50 p-4 text-sm leading-6 text-slate-700 dark:bg-slate-900 dark:text-slate-200">
              {detail.conversation?.summary ?? "לא נשמר סיכום חידוד לארוחה הזאת."}
            </div>
            ...
            {detail.messages.length > 0 ? (
              <div className="space-y-2">
                {detail.messages.map((message) => (
                  ...
                  >
                    <strong className="me-1">{message.role === "assistant" ? "AI:" : "אתם:"}</strong>
                    {message.content}
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-4">
          <Card className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle>תמונת הארוחה</CardTitle>
                <CardDescription>אם התמונה נשמרה בהצלחה, אפשר לראות אותה כאן.</CardDescription>
              </div>
            </div>
